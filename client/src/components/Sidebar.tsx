...
const Sidebar = ({ isMenuOpen, project, setProject, isGenerating, setIsGenerating }: SidebarProps) => {
  ...
  const handleRevisions = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const { data } = await api.post(`/api/project/revision/${project.id}`, { message: messageToSend });
      fetchProject();
      toast.success("Revision made successfully", {
      ...
      <div className='flex flex-col h-full'>
              ...
              if (isMessage) {
                ...
                return (
                  <div key={msg.id} className={`flex items-start gap-3
                    ...
                    {!isUser && (
                      <div className='w-8 h-8 flex items-center justify-center rounded-full bg-linear-to-br from-indigo-600 to-indigo-700'>
                ...
                return (
                  <div key={version.id} className='w-4/5 mx-auto my-2 p-3 rounded-xl bg-gray-800 text-gray-100 shadow flex flex-col gap-2'>
                    <div className='text-xs font-medium'>
                      code updated <br />
                      <span className='text-gray-500 text-sm font-normal'>{new Date(version.timestamp).toLocaleString()}</span>
                    </div>
        ...
        <form onSubmit={handleRevisions} action="" className='m-3 relative'>
          <div className='flex items-center gap-2'>
            <textarea onChange={(e) => setInput(e.target.value)} value={input} className='flex-1 p-3 rounded-xl resize-none text-sm outline-none ring ring-gray-700 focus:ring-indigo-500 bg-gray-800 text-gray-100 placeholder-gray-400 transition-all'
              rows={4} placeholder='Describe your website in details' disabled={isGenerating} />
            <button disabled={isGenerating || !input.trim()} className='absolute bottom-2.5 right-2.5 rounded-full bg-linear-to-r from-indigo-500 to-indigo-600 hover:from-indigo-600 hover:to-indigo-700 text-white transition-colors disabled:opacity-60'>
              {isGenerating ? <Loader2Icon className='size-7 p-1.5 text-white animate-spin' /> :
